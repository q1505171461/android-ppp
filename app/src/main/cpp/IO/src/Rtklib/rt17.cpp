    ...
    EXPORT int input_rt17f(raw_t *Raw, FILE *fp)
    {
        ...
        for (i = 0; i < 4096; i++)
        {
            if ((Data = fgetc(fp)) == EOF)
                return -2;
            if ((Ret = input_rt17(Raw, (uint8_t)Data)))
                return Ret;
        }
    ...
    */
    static int DecodeType17(raw_t *Raw, uint32_t rif)
    {
        ...
        for (i = n = 0; (i < nsat) && (i < MAXOBS); i++)
        {
            ...
            obs->code[0] = (obs->P[0] == 0.0) ? CODE_NONE : (Flags2 & M_BIT0) ? CODE_L1P
                                                                              : CODE_L1C;
            obs->code[1] = (obs->P[1] == 0.0) ? CODE_NONE : (Flags2 & M_BIT2) ? CODE_L2W
