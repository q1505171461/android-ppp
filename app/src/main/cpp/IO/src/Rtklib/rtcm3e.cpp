...
#define MIN(x, y) ((x) < (y) ? (x) : (y))
    ...
    int fcn_glo(int sat, rtcm_t *rtcm)
    {
        int prn, jsat;
        char cprn[256] = {0};
        ...
        // if (-1 != (jsat = pointer_string(dly->nprn, dly->cprn, cprn)))
        // {
        //     return dly->SAT[jsat].ifreq + 7;
        // }
        if (rtcm->nav.geph[prn - 1].sat == sat)
        {
            return rtcm->nav.geph[prn - 1].frq + 7;
        }
        if (rtcm->nav.glo_fcn[prn - 1] > 0)
        { /* fcn+8 (0: no data) */
                            ...
                            int *pr21, int *ppr2, int *lock2, int *cnr2)
    {
        ...
        *pr1 = *amb = 0;
        ...
        if (lock2)
            *lock2 = to_lock(lt2);
        if (cnr1)
            *cnr1 = ROUND(data->SNR[0] * SNR_UNIT / 0.25);
        if (cnr2)
            *cnr2 = ROUND(data->SNR[1] * SNR_UNIT / 0.25);
        if (code1)
    ...
    static int encode_head(int type, rtcm_t *rtcm, int sys, int sync, int nsat)
    {
        ...
        setbitu(rtcm->buff, i, 1, sync);
        i += 1; /* synchronous gnss flag */
        setbitu(rtcm->buff, i, 5, nsat);
    ...
    static int encode_type1001(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++)
        {
            ...
            setbitu(rtcm->buff, i, 6, prn);
            i += 6;
            setbitu(rtcm->buff, i, 1, code1);
            i += 1;
            setbitu(rtcm->buff, i, 24, pr1);
            i += 24;
            setbits(rtcm->buff, i, 20, ppr1);
            i += 20;
            setbitu(rtcm->buff, i, 7, lock1);
            i += 7;
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_type1002(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++)
        {
            ...
            setbitu(rtcm->buff, i, 7, lock1);
            i += 7;
            setbitu(rtcm->buff, i, 8, amb);
            i += 8;
            setbitu(rtcm->buff, i, 8, cnr1);
            i += 8;
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_type1003(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++)
        {
            ...
            setbitu(rtcm->buff, i, 7, lock1);
            i += 7;
            setbitu(rtcm->buff, i, 2, code2);
            i += 2;
            setbits(rtcm->buff, i, 14, pr21);
            i += 14;
            setbits(rtcm->buff, i, 20, ppr2);
            i += 20;
            setbitu(rtcm->buff, i, 7, lock2);
            i += 7;
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_type1007(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < n; j++)
        {
            setbitu(rtcm->buff, i, 8, rtcm->sta.antdes[j]);
            i += 8;
        }
        setbitu(rtcm->buff, i, 8, antsetup);
    ...
    static int encode_type1008(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < m; j++)
        {
            setbitu(rtcm->buff, i, 8, rtcm->sta.antsno[j]);
            i += 8;
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_type1009(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < rtcm->obs.n && nsat < MAXOBS; j++)
        {
            ...
            setbitu(rtcm->buff, i, 1, code1);
            i += 1;
            setbitu(rtcm->buff, i, 5, fcn);
            i += 5; /* fcn+7 */
            setbitu(rtcm->buff, i, 25, pr1);
            i += 25;
            setbits(rtcm->buff, i, 20, ppr1);
            i += 20;
            setbitu(rtcm->buff, i, 7, lock1);
    ...
    static int encode_type1019(rtcm_t *rtcm, int sync)
    {
        ...
        setbitu(rtcm->buff, i, 6, prn);
        i += 6;
        setbitu(rtcm->buff, i, 10, week);
        i += 10;
        setbitu(rtcm->buff, i, 4, eph->sva);
        i += 4;
        setbitu(rtcm->buff, i, 2, eph->code);
        i += 2;
        setbits(rtcm->buff, i, 14, idot);
        i += 14;
        setbitu(rtcm->buff, i, 8, eph->iode);
        i += 8;
        setbitu(rtcm->buff, i, 16, toc);
        i += 16;
        setbits(rtcm->buff, i, 8, af2);
        i += 8;
        setbits(rtcm->buff, i, 16, af1);
        i += 16;
        setbits(rtcm->buff, i, 22, af0);
        i += 22;
        setbitu(rtcm->buff, i, 10, eph->iodc);
        i += 10;
        setbits(rtcm->buff, i, 16, crs);
        i += 16;
        setbits(rtcm->buff, i, 16, deln);
        i += 16;
        setbits(rtcm->buff, i, 32, M0);
        i += 32;
        setbits(rtcm->buff, i, 16, cuc);
        i += 16;
        setbitu(rtcm->buff, i, 32, e);
        i += 32;
        setbits(rtcm->buff, i, 16, cus);
        i += 16;
        setbitu(rtcm->buff, i, 32, sqrtA);
        i += 32;
        setbitu(rtcm->buff, i, 16, toe);
        i += 16;
        setbits(rtcm->buff, i, 16, cic);
        i += 16;
        setbits(rtcm->buff, i, 32, OMG0);
        i += 32;
        setbits(rtcm->buff, i, 16, cis);
        i += 16;
        setbits(rtcm->buff, i, 32, i0);
        i += 32;
        setbits(rtcm->buff, i, 16, crc);
        i += 16;
        setbits(rtcm->buff, i, 32, omg);
        i += 32;
        setbits(rtcm->buff, i, 24, OMGd);
        i += 24;
        setbits(rtcm->buff, i, 8, tgd);
        i += 8;
        setbitu(rtcm->buff, i, 6, eph->svh);
        i += 6;
    ...
    static int encode_type1020(rtcm_t *rtcm, int sync)
    {
        ...
        setbitu(rtcm->buff, i, 5, tk_h);
        i += 5;
        setbitu(rtcm->buff, i, 6, tk_m);
        i += 6;
        setbitu(rtcm->buff, i, 1, tk_s);
        i += 1;
        setbitu(rtcm->buff, i, 1, geph->svh);
        i += 1; /* Bn */
        setbitu(rtcm->buff, i, 1, 0);
        i += 1; /* P2 */
        setbitu(rtcm->buff, i, 7, tb);
        i += 7;
        setbitg(rtcm->buff, i, 24, vel[0]);
        i += 24;
        setbitg(rtcm->buff, i, 27, pos[0]);
        i += 27;
        setbitg(rtcm->buff, i, 5, acc[0]);
        i += 5;
        setbitg(rtcm->buff, i, 24, vel[1]);
        i += 24;
        setbitg(rtcm->buff, i, 27, pos[1]);
        i += 27;
        setbitg(rtcm->buff, i, 5, acc[1]);
        i += 5;
        setbitg(rtcm->buff, i, 24, vel[2]);
        i += 24;
        setbitg(rtcm->buff, i, 27, pos[2]);
        i += 27;
        setbitg(rtcm->buff, i, 5, acc[2]);
        i += 5;
        setbitu(rtcm->buff, i, 1, 0);
        i += 1; /* P3 */
        setbitg(rtcm->buff, i, 11, gamn);
        i += 11;
        setbitu(rtcm->buff, i, 3, 0);
        i += 3; /* P,ln */
        setbitg(rtcm->buff, i, 22, taun);
        i += 22;
        setbitg(rtcm->buff, i, 5, dtaun);
        i += 5;
        setbitu(rtcm->buff, i, 5, geph->age);
        i += 5; /* En */
        setbitu(rtcm->buff, i, 1, 0);
        i += 1; /* P4 */
        setbitu(rtcm->buff, i, 4, 0);
        i += 4; /* FT */
        setbitu(rtcm->buff, i, 11, NT);
        i += 11;
        setbitu(rtcm->buff, i, 2, 0);
        i += 2; /* M */
        setbitu(rtcm->buff, i, 1, 0);
    ...
    static int encode_type1033(rtcm_t *rtcm, int sync)
    {
        ...
        for (j = 0; j < I; j++)
        {
            setbitu(rtcm->buff, i, 8, rtcm->sta.rectype[j]);
            i += 8;
        }
        ...
        for (j = 0; j < J; j++)
        {
            setbitu(rtcm->buff, i, 8, rtcm->sta.recver[j]);
            i += 8;
        }
        ...
        for (j = 0; j < K; j++)
        {
            setbitu(rtcm->buff, i, 8, rtcm->sta.recsno[j]);
            i += 8;
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_type1041(rtcm_t *rtcm, int sync)
    {
        ...
        setbits(rtcm->buff, i, 8, tgd);
        i += 8;
        setbits(rtcm->buff, i, 22, deln);
        i += 22;
        setbitu(rtcm->buff, i, 8, eph->iode);
        i += 8 + 10; /* IODEC */
        setbitu(rtcm->buff, i, 2, eph->svh);
        i += 2; /* L5+Sflag */
        setbits(rtcm->buff, i, 15, cuc);
        i += 15;
        setbits(rtcm->buff, i, 15, cus);
        i += 15;
        setbits(rtcm->buff, i, 15, cic);
        i += 15;
        setbits(rtcm->buff, i, 15, cis);
        i += 15;
        setbits(rtcm->buff, i, 15, crc);
        i += 15;
        setbits(rtcm->buff, i, 15, crs);
        i += 15;
        setbits(rtcm->buff, i, 14, idot);
        i += 14;
        setbits(rtcm->buff, i, 32, M0);
    ...
    static int encode_type1045(rtcm_t *rtcm, int sync)
    {
        ...
        if (satsys(rtcm->ephsat, &prn) != SYS_GAL)
            return 0;
        eph = rtcm->nav.eph + rtcm->ephsat - 1 + MAXRTKSAT; /* F/NAV */
        if (eph->sat != rtcm->ephsat)
            return 0;
        ...
        setbitu(rtcm->buff, i, 6, prn);
        i += 6;
        setbitu(rtcm->buff, i, 12, week);
        i += 12;
        setbitu(rtcm->buff, i, 10, eph->iode);
        i += 10;
        setbitu(rtcm->buff, i, 8, eph->sva);
        i += 8;
        setbits(rtcm->buff, i, 14, idot);
        i += 14;
        setbitu(rtcm->buff, i, 14, toc);
        i += 14;
        setbits(rtcm->buff, i, 6, af2);
        i += 6;
        setbits(rtcm->buff, i, 21, af1);
        i += 21;
        setbits(rtcm->buff, i, 31, af0);
        i += 31;
        setbits(rtcm->buff, i, 16, crs);
        i += 16;
        setbits(rtcm->buff, i, 16, deln);
        ...
        setbits(rtcm->buff, i, 24, OMGd);
        i += 24;
        setbits(rtcm->buff, i, 10, bgd1);
        i += 10;
        setbitu(rtcm->buff, i, 2, oshs);
        i += 2; /* E5a SVH */
        setbitu(rtcm->buff, i, 1, osdvs);
        i += 1; /* E5a DVS */
        setbitu(rtcm->buff, i, 7, 0);
        i += 7; /* reserved */
        rtcm->nbit = i;
    ...
    static int encode_type1046(rtcm_t *rtcm, int sync)
    {
        ...
        setbits(rtcm->buff, i, 10, bgd1);
        i += 10;
        setbits(rtcm->buff, i, 10, bgd2);
        i += 10;
        setbitu(rtcm->buff, i, 2, oshs1);
        i += 2; /* E5b SVH */
        setbitu(rtcm->buff, i, 1, osdvs1);
        i += 1; /* E5b DVS */
        setbitu(rtcm->buff, i, 2, oshs2);
        i += 2; /* E1 SVH */
        setbitu(rtcm->buff, i, 1, osdvs2);
        i += 1; /* E1 DVS */
        rtcm->nbit = i;
        return 1;
    }
    ...
    static int encode_type1042(rtcm_t *rtcm, int sync)
    {
        ...
        setbitu(rtcm->buff, i, 6, prn);
        i += 6;
        setbitu(rtcm->buff, i, 13, week);
        i += 13;
        setbitu(rtcm->buff, i, 4, eph->sva);
        i += 4;
        setbits(rtcm->buff, i, 14, idot);
        i += 14;
        setbitu(rtcm->buff, i, 5, eph->iode);
        i += 5;
        setbitu(rtcm->buff, i, 17, toc);
        i += 17;
        setbits(rtcm->buff, i, 11, af2);
        i += 11;
        setbits(rtcm->buff, i, 22, af1);
        i += 22;
        setbits(rtcm->buff, i, 24, af0);
        i += 24;
        setbitu(rtcm->buff, i, 5, eph->iodc);
        i += 5;
        setbits(rtcm->buff, i, 18, crs);
        i += 18;
        setbits(rtcm->buff, i, 16, deln);
        i += 16;
        setbits(rtcm->buff, i, 32, M0);
        i += 32;
        setbits(rtcm->buff, i, 18, cuc);
        i += 18;
        setbitu(rtcm->buff, i, 32, e);
        i += 32;
        setbits(rtcm->buff, i, 18, cus);
        i += 18;
        setbitu(rtcm->buff, i, 32, sqrtA);
        i += 32;
        setbitu(rtcm->buff, i, 17, toe);
        i += 17;
        setbits(rtcm->buff, i, 18, cic);
        i += 18;
        setbits(rtcm->buff, i, 32, OMG0);
        i += 32;
        setbits(rtcm->buff, i, 18, cis);
        i += 18;
        setbits(rtcm->buff, i, 32, i0);
        i += 32;
        setbits(rtcm->buff, i, 18, crc);
        i += 18;
        setbits(rtcm->buff, i, 32, omg);
        i += 32;
        setbits(rtcm->buff, i, 24, OMGd);
        i += 24;
        setbits(rtcm->buff, i, 10, tgd1);
        i += 10;
        setbits(rtcm->buff, i, 10, tgd2);
        i += 10;
        setbitu(rtcm->buff, i, 1, eph->svh);
        i += 1;
        rtcm->nbit = i;
        return 1;
    }
                               ...
                               int provid, int solid)
    {
        ...
        setbitu(rtcm->buff, i, 4, udi);
        i += 4; /* update interval */
        setbitu(rtcm->buff, i, 1, sync);
        i += 1; /* multiple message indicator */
        if (subtype == 0 && (type == 1 || type == 4))
        {
            setbitu(rtcm->buff, i, 1, refd);
            i += 1; /* satellite ref datum */
        }
        setbitu(rtcm->buff, i, 4, iod);
        i += 4; /* IOD SSR */
        setbitu(rtcm->buff, i, 16, provid);
        i += 16; /* provider ID */
        setbitu(rtcm->buff, i, 4, solid);
        i += 4; /* solution ID */
        if (subtype > 0 && (type == 1 || type == 4))
        {
            setbitu(rtcm->buff, i, 1, refd);
    ...
    static int encode_ssr1(rtcm_t *rtcm, int sys, int subtype, int sync)
    {
        ...
        {
            if (satsys(j + 1, &prn) != sys || !rtcm->ssr[j].update)
                continue;
            nsat++;
        ...
        for (j = 0; j < MAXRTKSAT; j++)
        {
            ...
            i += np; /* satellite ID */
            setbitu(rtcm->buff, i, ni, iode);
            i += ni; /* IODE */
            setbitu(rtcm->buff, i, nj, iodcrc);
            i += nj; /* IODCRC */
            setbits(rtcm->buff, i, 22, deph[0]);
            i += 22; /* delta radial */
            setbits(rtcm->buff, i, 20, deph[1]);
            i += 20; /* delta along-track */
            setbits(rtcm->buff, i, 20, deph[2]);
            i += 20; /* delta cross-track */
            setbits(rtcm->buff, i, 21, ddeph[0]);
            i += 21; /* dot delta radial */
            setbits(rtcm->buff, i, 19, ddeph[1]);
            i += 19; /* dot delta along-track */
            setbits(rtcm->buff, i, 19, ddeph[2]);
    ...
    static int encode_ssr2(rtcm_t *rtcm, int sys, int subtype, int sync)
    {
        ...
        for (j = 0; j < MAXRTKSAT; j++)
        {
            ...
            setbitu(rtcm->buff, i, np, prn - offp);
            i += np; /* satellite ID */
            setbits(rtcm->buff, i, 22, dclk[0]);
            i += 22; /* delta clock C0 */
            setbits(rtcm->buff, i, 21, dclk[1]);
            i += 21; /* delta clock C1 */
            setbits(rtcm->buff, i, 27, dclk[2]);
            i += 27; /* delta clock C2 */
        }
        rtcm->nbit = i;
        return 1;
    ...
    static int encode_ssr3(rtcm_t *rtcm, int sys, int subtype, int sync)
    {
        ...
        for (j = nsat = 0; j < MAXRTKSAT; j++)
        {
            ...
            {
                if (!codes[k] || rtcm->ssr[j].cbias[codes[k] - 1] == 0.0)
                    continue;
                code[nbias] = k;
            ...
            for (k = 0; k < nbias; k++)
            {
                setbitu(rtcm->buff, i, 5, code[k]);
                i += 5; /* signal indicator */
                setbits(rtcm->buff, i, 14, bias[k]);
                i += 14; /* code bias */
            }
        }
        rtcm->nbit = i;
    ...
    static int encode_ssr7(rtcm_t *rtcm, int sys, int subtype, int sync)
    {
        ...
        for (j = nsat = 0; j < MAXRTKSAT; j++)
        {
            ...
            for (k = nbias = 0; k < 32; k++)
            {
                if (!codes[k] || rtcm->ssr[j].pbias[codes[k] - 1] == 0.0)
                    continue;
                code[nbias] = k;
            ...
            setbitu(rtcm->buff, i, 5, nbias);
            i += 5; /* number of code biases */
            setbitu(rtcm->buff, i, 9, yaw_ang);
            i += 9; /* yaw angle */
            setbits(rtcm->buff, i, 8, yaw_rate);
            i += 8; /* yaw rate */
            ...
            for (k = 0; k < nbias; k++)
            {
                ...
                setbitu(rtcm->buff, i, 4, 0);
                i += 4; /* discont counter */
                setbits(rtcm->buff, i, 20, pbias[k]);
                i += 20; /* phase bias */
                if (subtype == 0)
                {
                    setbits(rtcm->buff, i, 17, stdpb[k]);
                              ...
                              uint8_t *cell_ind)
    {
        ...
        *nsat = *nsig = *ncell = 0;
        ...
        {
            if (!(sat = to_satid(sys, rtcm->obs.data[i].sat)))
                continue;

            for (j = 0; j < NFREQ + NEXOBS; j++)
            {
                if (!(sig = to_sigid(sys, rtcm->obs.data[i].code[j])))
                    continue;
                            ...
                            double *rrng, double *rrate, uint8_t *info)
    {
        ...
        for (i = 0; i < rtcm->obs.n; i++)
        {
            ...
            if (!(sat = to_satid(sys, data->sat)))
                continue;

            for (j = 0; j < NFREQ + NEXOBS; j++)
            {
                if (!(sig = to_sigid(sys, data->code[j])))
                    continue;
                k = sat_ind[sat - 1] - 1;
                            ...
                            double *rate, double *lock, uint8_t *half, float *cnr)
    {
        ...
        for (i = 0; i < rtcm->obs.n; i++)
        {
            ...
            for (j = 0; j < NFREQ + NEXOBS; j++)
            {
                ...
                if (psrng && psrng_s != 0.0)
                    psrng[cell - 1] = psrng_s;
                if (phrng && phrng_s != 0.0)
                ...
                if (cnr)
                    cnr[cell - 1] = (float)(data->SNR[j] * SNR_UNIT);
            }
        }
    }
    /* encode MSM header ---------------------------------------------------------*/
                               ...
                               float *cnr)
    {
        ...
        setbitu(rtcm->buff, i, 3, rtcm->seqno);
        ...
        for (j = 0; j < 64; j++)
        {
            setbitu(rtcm->buff, i, 1, sat_ind[j] ? 1 : 0);
            i += 1;
        }
        /* signal mask */
        for (j = 0; j < 32; j++)
        {
            setbitu(rtcm->buff, i, 1, sig_ind[j] ? 1 : 0);
            i += 1;
        }
        /* cell mask */
        for (j = 0; j < *nsat * nsig && j < 64; j++)
        {
            setbitu(rtcm->buff, i, 1, cell_ind[j] ? 1 : 0);
            i += 1;
        }
        /* generate msm satellite data fields */
    ...
    static int encode_type1230(rtcm_t *rtcm, int sync)
    {
        ...
        setbitu(rtcm->buff, i, 12, rtcm->staid);
        i += 12; /* station ID */
        setbitu(rtcm->buff, i, 1, align);
        i += 1; /* GLO code-phase bias ind */
        setbitu(rtcm->buff, i, 3, 0);
        i += 3; /* reserved */
        setbitu(rtcm->buff, i, 4, mask);
        i += 4; /* GLO FDMA signals mask */
        setbits(rtcm->buff, i, 16, bias[0]);
        i += 16; /* GLO C1 code-phase bias */
        setbits(rtcm->buff, i, 16, bias[1]);
        i += 16; /* GLO P1 code-phase bias */
        setbits(rtcm->buff, i, 16, bias[2]);
        i += 16; /* GLO C2 code-phase bias */
        setbits(rtcm->buff, i, 16, bias[3]);
