...
#define P2_66 1.355252715606880E-20 /* 2^-66 */
    ...
    const uint8_t ssr_sig_cmp[32] = {
        ...
        0, CODE_L1D, CODE_L1P, 0, CODE_L5D, CODE_L5P, CODE_L5X, CODE_L1A,
        0, 0, CODE_L6A};
    ...
    static int decode_type1029(rtcm_t *rtcm)
    {
        ...
        if (i + 60 <= rtcm->len * 8)
        {
            staid = getbitu(rtcm->buff, i, 12);
            i += 12;
            mjd = getbitu(rtcm->buff, i, 16);
            i += 16;
            tod = getbitu(rtcm->buff, i, 17);
            i += 17;
            nchar = getbitu(rtcm->buff, i, 7);
            i += 7;
            cunit = getbitu(rtcm->buff, i, 8);
            i += 8;
        }
        else
        {
    ...
    static int decode_type1041(rtcm_t *rtcm)
    {
        ...
        if (i + 482 - 12 <= rtcm->len * 8)
        {
            ...
            eph.tgd[0] = getbits(rtcm->buff, i, 8) * P2_31;
            i += 8;
            eph.deln = getbits(rtcm->buff, i, 22) * P2_41 * SC2RAD;
            i += 22;
            eph.iode = getbitu(rtcm->buff, i, 8);
            i += 8 + 10; /* IODEC */
            eph.svh = getbitu(rtcm->buff, i, 2);
            i += 2; /* L5+Sflag */
            eph.cuc = getbits(rtcm->buff, i, 15) * P2_28;
            i += 15;
            eph.cus = getbits(rtcm->buff, i, 15) * P2_28;
            i += 15;
            eph.cic = getbits(rtcm->buff, i, 15) * P2_28;
            i += 15;
            eph.cis = getbits(rtcm->buff, i, 15) * P2_28;
            i += 15;
            eph.crc = getbits(rtcm->buff, i, 15) * 0.0625;
            i += 15;
            eph.crs = getbits(rtcm->buff, i, 15) * 0.0625;
            i += 15;
            eph.idot = getbits(rtcm->buff, i, 14) * P2_43 * SC2RAD;
            i += 14;
            eph.M0 = getbits(rtcm->buff, i, 32) * P2_31 * SC2RAD;
            ...
            eph.omg = getbits(rtcm->buff, i, 32) * P2_31 * SC2RAD;
            i += 32;
            eph.OMGd = getbits(rtcm->buff, i, 22) * P2_41 * SC2RAD;
            i += 22;
            eph.i0 = getbits(rtcm->buff, i, 32) * P2_31 * SC2RAD;
        }
        else
    ...
    static int decode_type1045(rtcm_t *rtcm)
    {
        ...
        if (!strstr(rtcm->opt, "-EPHALL"))
        {
            if (eph.iode == rtcm->nav.eph[sat - 1 + MAXRTKSAT].iode)
                return 0; /* unchanged */
        }
        rtcm->nav.eph[sat - 1 + MAXRTKSAT] = eph;
        rtcm->ephsat = sat;
        rtcm->ephset = 1; /* F/NAV */
        return 2;
    }
    ...
    /* decode SSR 8: high ifpb  correction ----------------------------------*/
    static int decode_ssr8(rtcm_t *rtcm, int sys, int subtype)
    {
        double udint, ifpb, tow, tow_p;
        int i, j, type, nsat, sync, iod, prn, sat, np, offp, week;
        ...
        for (j = 0; j < nsat && i + 22 + np <= rtcm->len * 8; j++)
        {
            prn = getbitu(rtcm->buff, i, np) + offp;
            i += np;
            ifpb = getbits(rtcm->buff, i, 22) * 1E-4;
            i += 22;
            tow = getbitu(rtcm->buff, i, 20);
            i += 20;

            gtime_t t_ifpb0 = utc2gpst(timeget()); /* priori time */
            tow_p = time2gpst(t_ifpb0, &week);
            if (tow < tow_p - 302400.0)
                tow += 604800.0;
            else if (tow > tow_p + 302400.0)
                tow -= 604800.0;
            t_ifpb0 = gpst2time(week, tow); /* exact time */
            ...
            rtcm->ssr[sat - 1].t0[6] = rtcm->time;
            rtcm->ssr[sat - 1].udi[6] = udint;
            rtcm->ssr[sat - 1].iod[6] = iod;
            rtcm->ssr[sat - 1].ifpb = ifpb;
            rtcm->ssr[sat - 1].ifpb_t0 = t_ifpb0;
            rtcm->ssr[sat - 1].update = 1;
        }
        return sync ? 0 : 20;
    }
    ...
    static int decode_ssr7(rtcm_t *rtcm, int sys, int subtype)
    {
        ...
        for (j = 0; j < nsat && i + 5 + 17 + np <= rtcm->len * 8; j++)
        {
            ...
            for (k = 0; k < nbias && i + ((subtype == 0) ? 49 : 32) <= rtcm->len * 8; k++)
            {
                mode = getbitu(rtcm->buff, i, 5);
                i += 5;
                sii = getbitu(rtcm->buff, i, 1);
                i += 1; /* integer-indicator */
                swl = getbitu(rtcm->buff, i, 2);
                i += 2; /* WL integer-indicator */
                sdc = getbitu(rtcm->buff, i, 4);
                ...
                {
                    std = 100; /// for identified  // add by jtao
                }
                if (sigs[mode])
                {
            ...
            rtcm->ssr[sat - 1].update = 1;                           // add by jtao
                             ...
                             const int *ex, const int *half)
    {
        ...
        for (i = j = 0; i < h->nsat; i++)
        {
            ...
            if (sys == SYS_GLO)
            {
                ...
                // }
                //// here should update here
                fcn = fcn_glo(sat, rtcm) == -1 ? fcn : fcn_glo(sat, rtcm) - 7;
            }
            for (k = 0; k < h->nsig; k++)
            {
    ...
    static int decode_type1230(rtcm_t *rtcm)
    {
        ...
        staid = getbitu(rtcm->buff, i, 12);
        i += 12;
        align = getbitu(rtcm->buff, i, 1);
        i += 1 + 3;
        mask = getbitu(rtcm->buff, i, 4);
        i += 4;
    ...
    static int decode_type4076(rtcm_t *rtcm)
    {
        ...
        case 161:
            return decode_ssr8(rtcm, SYS_GPS, subtype);
        }
