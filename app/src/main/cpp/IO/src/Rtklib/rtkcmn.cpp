...
#define MAX_VAR_EPH SQR(300.0) /* max variance eph to reject satellite (m^2) */
    ...
    // static char codepris[7][MAXRTKFREQ][16] = {
    ...
    static char codepris[7][MAXRTKFREQ][16] = {
        ...
        {"CLSXZ", "LSX", "IQXDPZ", "LSXEZ", "", "", ""},            /* QZS */
        {"C", "IQX", "", "", "", "", ""},                           /* SBS */
        {"IQXDPAN", "IQXDPAN", "IQXDPZ", "DPX", "IQXA", "DPX", ""}, /* BDS */
        {"ABCX", "ABCX", "", "", "", "", ""}                        /* IRN */
    };
...
#define dgetrs_ dgetrs
    ...
    { /// index to P/D/L,should change B1/B1c position if using B1c/B2a/B2b
        double freq;
    ...
    extern gtime_t timeget(void)
    {
        ...
        // double ep_x[] = { 2021,10,10,15,20,0 };
        // return epoch2time(ep_x);
    }
    ...
    extern double utc2gmst(gtime_t t, double ut1_utc)
    {
        ...
        return fmod(gmst, 86400.0) * rtklib_pi / 43200.0; /* 0 <= gmst <= 2*pi */
    }
    /* time to string --------------------------------------------------------------
    ...
    extern void ecef2pos(const double *r, double *pos)
    {
        ...
        pos[0] = r2 > 1E-12 ? atan(z / sqrt(r2)) : (r[2] > 0.0 ? rtklib_pi / 2.0 : -rtklib_pi / 2.0);
...
#define Rz(t, X)                                 \
    ...
    static void ast_args(double t, double *f)
    {
        ...
        for (i = 0; i < 5; i++)
        {
            ...
            for (j = 0; j < 4; j++)
                f[i] += fc[i][j + 1] * tt[j];
            f[i] = fmod(f[i] * AS2R, 2.0 * rtklib_pi);
        }
    }
    /* iau 1980 nutation ---------------------------------------------------------*/
    ...
    static int readantex(const char *file, pcvs_t *pcvs)
    {
        ...
        while (fgets(buff, sizeof(buff), fp))
        {
            ...
            {
                if (!pcv.sat && buff[3] != 'G')
                    continue; /* only read rec ant for GPS */
    ...
    extern int expath(const char *path, char *paths[], int nmax)
    {
        ...
        if ((c_p = strrchr(path, '/')) || (c_p = strrchr(path, '\\')))
        {
            file = c_p + 1;
            strncpy(dir, path, c_p - path + 1);
            dir[c_p - path + 1] = '\0';
        }
    ...
    {
        double az = 0.0, el = rtklib_pi / 2.0, enu[3];
...
#define SQRT(x) ((x) < 0.0 || (x) != (x) ? 0.0 : sqrt(x))
                           ...
                           const double *azel)
    {
        ...
        psi = 0.0137 / (azel[1] / rtklib_pi + 0.11) - 0.022;

        /* subionospheric latitude/longitude (semi-circle) */
        phi = pos[0] / rtklib_pi + psi * cos(azel[0]);
        if (phi > 0.416)
            phi = 0.416;
        else if (phi < -0.416)
            phi = -0.416;
        lam = pos[1] / rtklib_pi + psi * sin(azel[0]) / cos(phi * rtklib_pi);

        /* geomagnetic latitude (semi-circle) */
        phi += 0.064 * cos((lam - 1.617) * rtklib_pi);
        ...
        /* slant factor */
        f = 1.0 + 16.0 * pow(0.53 - azel[1] / rtklib_pi, 3.0);
        ...
        x = 2.0 * rtklib_pi * (tt - 50400.0) / per;
    ...
    extern double ionmapf(const double *pos, const double *azel)
    {
        if (pos[2] >= HION)
            return 1.0;
        return 1.0 / cos(asin((RE_WGS84 + pos[2]) / (RE_WGS84 + HION) * sin(rtklib_pi / 2.0 - azel[1])));
    }
    /* ionosphericrtklib_pierce point position -------------------------------------------
     * compute ionosphericrtklib_pierce point (ipp) position and slant factor
     * args   : double *pos      I   receiver position {lat,lon,h} (rad,m)
     *          double *azel     I   azimuth/elevation angle {az,el} (rad)
     *          double re        I   earth radius (km)
     *          double hion      I   altitude of ionosphere (km)
     *          double *posp     O  rtklib_pierce point position {lat,lon,h} (rad,m)
     * return : slant factor
                         ...
                         double hion, double *posp)
    {
        ...
        ap = rtklib_pi / 2.0 - azel[1] - asin(rp);
        sinap = sin(ap);
        tanap = tan(ap);
        cosaz = cos(azel[0]);
        posp[0] = asin(sin(pos[0]) * cos(ap) + cos(pos[0]) * sinap * cosaz);

        if ((pos[0] > 70.0 * D2R && tanap * cosaz > tan(rtklib_pi / 2.0 - pos[0])) ||
            (pos[0] < -70.0 * D2R && -tanap * cosaz > tan(rtklib_pi / 2.0 + pos[0])))
        {
            posp[1] = pos[1] + rtklib_pi - asin(sinap * sin(azel[0]) / cos(posp[0]));
        }
        else
        {
