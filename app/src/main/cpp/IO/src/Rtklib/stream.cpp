...
#define closesocket close
    ...
    typedef struct
    {                               /* ntrip client/server connection type */
    ...
    static serial_t *openserial(const char *path, int mode, char *msg)
    {
        ...
        if (!(serial = (serial_t *)calloc(1, sizeof(serial_t))))
            return NULL;
    ...
    static int writeserial(serial_t *serial, uint8_t *buff, int n, char *msg)
    {
        ...
        if (!serial)
            return 0;
#ifdef WIN32
        if ((ns = writeseribuff(serial, buff, n)) < n)
            serial->error = 1;
#else
    ...
    static tcpsvr_t *opentcpsvr(const char *path, char *msg)
    {
        ...
        if (!(tcpsvr = (tcpsvr_t *)malloc(sizeof(tcpsvr_t))))
            return NULL;
        *tcpsvr = tcpsvr0;
    ...
    static ntripc_t *openntripc(const char *path, char *msg)
    {
        ...
        if (!(ntripc = (ntripc_t *)malloc(sizeof(ntripc_t))))
            return NULL;
    ...
    static void wait_ntripc(ntripc_t *ntripc, char *msg)
    {
        ...
        if (!waittcpsvr(ntripc->tcp, msg))
            return;

        for (i = 0; i < MAXCLI; i++)
        {
            if (ntripc->tcp->cli[i].state != 2 || ntripc->con[i].state)
                continue;
    ...
    static udp_t *genudp(int type, int port, const char *saddr, char *msg)
    {
        ...
        if (!(udp = (udp_t *)malloc(sizeof(udp_t))))
            return NULL;
        udp->state = 2;
        udp->type = type;
        udp->port = port;
...
#endif
    {
        ...
        char cmd[5120], env[1024] = "", opt[1024], *proxyopt = "";
        const char *proto;
        int ret;
    ...
    static ftp_t *openftp(const char *path, int type, char *msg)
    {
        ...
        if (!(ftp = (ftp_t *)malloc(sizeof(ftp_t))))
            return NULL;
    ...
    static membuf_t *openmembuf(const char *path, char *msg)
    {
        ...
        if (!(membuf = (membuf_t *)malloc(sizeof(membuf_t))))
            return NULL;
        membuf->state = 1;
        membuf->rp = 0;
        membuf->wp = 0;
    ...
    extern int stropen(stream_t *stream, int type, int mode, const char *path)
    {
        ...
        case STR_UDPSVR:
            ...
            break;
        case STR_UDPCLI:
            stream->port = openudpcli(path, stream->msg);
            break;
        case STR_MEMBUF:
    ...
    extern int strread(stream_t *stream, uint8_t *buff, int n)
    {
        ...
        case STR_NTRIPCLI:
            ...
            break;
        case STR_NTRIPCAS:
            nr = readntripc((ntripc_t *)stream->port, buff, n, msg);
            break;
        case STR_UDPSVR:
            nr = readudpsvr((udp_t *)stream->port, buff, n, msg);
            break;
        case STR_MEMBUF:
            nr = readmembuf((membuf_t *)stream->port, buff, n, msg);
            break;
        case STR_FTP:
    ...
    extern int strwrite(stream_t *stream, uint8_t *buff, int n)
    {
        ...
        case STR_NTRIPCLI:
            ...
            break;
        case STR_NTRIPCAS:
            ns = writentripc((ntripc_t *)stream->port, buff, n, msg);
            break;
        case STR_UDPCLI:
            ns = writeudpcli((udp_t *)stream->port, buff, n, msg);
            break;
        case STR_MEMBUF:
            ns = writemembuf((membuf_t *)stream->port, buff, n, msg);
            break;
        case STR_FTP:
        case STR_HTTP:
        default:
    ...
    extern int strstatx(stream_t *stream, char *msg)
    {
        ...
        {
        case STR_SERIAL:
            state = statexserial((serial_t *)stream->port, msg);
            break;
        case STR_FILE:
            state = statexfile((file_t *)stream->port, msg);
            break;
        case STR_TCPSVR:
            state = statextcpsvr((tcpsvr_t *)stream->port, msg);
            break;
        case STR_TCPCLI:
            state = statextcpcli((tcpcli_t *)stream->port, msg);
            break;
        case STR_NTRIPSVR:
        case STR_NTRIPCLI:
            state = statexntrip((ntrip_t *)stream->port, msg);
            break;
        case STR_NTRIPCAS:
            state = statexntripc((ntripc_t *)stream->port, msg);
            break;
        case STR_UDPSVR:
            state = statexudpsvr((udp_t *)stream->port, msg);
            break;
        case STR_UDPCLI:
            state = statexudpcli((udp_t *)stream->port, msg);
            break;
        case STR_MEMBUF:
            state = statexmembuf((membuf_t *)stream->port, msg);
            break;
        case STR_FTP:
            state = statexftp((ftp_t *)stream->port, msg);
            break;
        case STR_HTTP:
            state = statexftp((ftp_t *)stream->port, msg);
            break;
    ...
    extern void strsendcmd(stream_t *str, const char *cmd)
    {
        ...
        for (;;)
        {
            ...
            { /* binary escape */
                ...
                else if (!strncmp(msg + 1, "BRATE", 5))
                { /* set bitrate */
                    if (sscanf(msg + 6, "%d", &brate) < 1)
                        brate = 9600;
                    set_brate(str, brate);
                    sleepms(500);
                }
