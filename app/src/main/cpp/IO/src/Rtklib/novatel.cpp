...
#define I1(p) (*((int8_t *)(p)))
    ...
    static int decode_galephemerisb(raw_t *raw)
    {
        ...
        if (!strstr(raw->opt, "-EPHALL"))
        {
            if (eph.iode == raw->nav.eph[sat - 1 + MAXRTKSAT * set].iode &&
                timediff(eph.toe, raw->nav.eph[sat - 1 + MAXRTKSAT * set].toe) == 0.0 &&
                timediff(eph.toc, raw->nav.eph[sat - 1 + MAXRTKSAT * set].toc) == 0.0)
            {
                return 0; /* unchanged */
            }
        }
    ...
    static int decode_navicephemerisb(raw_t *raw)
    {
        ...
        {
            if (timediff(raw->nav.eph[sat - 1].toe, eph.toe) == 0.0 &&
                raw->nav.eph[sat - 1].iode == eph.iode)
                return 0; /* unchanged */
        }
        raw->nav.eph[sat - 1] = eph;
        raw->ephsat = sat;
